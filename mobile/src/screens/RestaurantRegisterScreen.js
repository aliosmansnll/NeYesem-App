...
export default function RestaurantRegisterScreen({ navigation }) {
  ...
  const handleMapPress = async (location) => {
    // MapComponent'ten gelen location = { latitude, longitude }
                  ...
                  <View style={styles.coordinatesInfo}>
                    ...
                    <View style={{ flex: 1 }}>
                      <Text style={styles.coordinatesText}>
                        📍 {(formData.ilce || formData.sehir) ? [formData.ilce, formData.sehir].filter(Boolean).join(', ') : 'Konum seçildi'}
                      </Text>
